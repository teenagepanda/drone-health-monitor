        ...
        if rc_rssi is not None and rc_rssi != 255:
            if rc_rssi >= 60:
                ...
                add_result(rows, plain_lines, problems, "שלט RC / ELRS", "⚠️ גבולי",
                           f"זוהו {rc_channels:.0f} ערוצים אך RSSI ממוצע נמוך: {rc_rssi:.0f}.", True, False)
            else:
                add_result(rows, plain_lines, problems, "שלט RC / ELRS", "❌ לא תקין",
                           f"RSSI נמוך מאוד: {rc_rssi:.0f}.", True)
        else:
            add_result(rows, plain_lines, problems, "שלט RC / ELRS", "✅ תקין",
                       f"זוהו {rc_channels:.0f} ערוצים. RSSI לא מדווח, אך ערוצי RC מתקבלים.")
    else:
        add_result(rows, plain_lines, problems, "שלט RC / ELRS", "❌ לא תקין",
                   f"זוהו רק {rc_channels:.0f} ערוצים. נדרשים לפחות 4 ערוצים.", True)

    fs_thr = avg(df, "fs_thr_enable")
    fs_batt = avg(df, "fs_batt_enable")
    fs_gcs = avg(df, "fs_gcs_enable")
    fs_ekf = avg(df, "fs_ekf_action")
    rtl_alt = avg(df, "rtl_alt")

    fs_missing = all(v is None for v in [fs_thr, fs_batt, fs_gcs, fs_ekf])
    if fs_missing:
        add_result(rows, plain_lines, problems, "Failsafe Configuration", "⚠️ חסר מידע",
                   "לא התקבלו פרמטרי Failsafe מהבקר.", True, False)
    else:
        notes = []

        # These values are configuration parameters only. They do not indicate
        # that a failsafe is currently active.
        if fs_thr is None:
            notes.append("RC Failsafe: לא ידוע")
        elif fs_thr == 0:
            notes.append("RC Failsafe protection: disabled")
        else:
            notes.append(f"RC Failsafe protection: enabled (FS_THR_ENABLE={int(fs_thr)})")

        if fs_batt is None:
            notes.append("Battery Failsafe: לא ידוע")
        elif fs_batt == 0:
            notes.append("Battery Failsafe protection: disabled")
        else:
            notes.append(f"Battery Failsafe protection: enabled (FS_BATT_ENABLE={int(fs_batt)})")
        ...
        add_result(
            ...
            "ℹ️ מידע",
            "; ".join(notes)
            + ". אלו הגדרות בלבד ואינן מעידות ש-Failsafe פעיל כרגע."
        )
